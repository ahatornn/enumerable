...
| [Contains](./contains.go) | Determines whether a sequence contains a specified element using the provided equality comparer. This operation is useful when you need to check for the existence of a specific value in the sequence with custom equality logic.<br/><br/>⚠️ Performance note: This is a terminal operation that must iterate through the sequence until the element is found or sequence ends.<br/>⚠️ Memory note: This operation does not buffer elements.<br/>ℹ️ Available only for non-comparable types. |
